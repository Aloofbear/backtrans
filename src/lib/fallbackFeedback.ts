...
export function createLocalFeedback(input: FallbackInput): AnalysisFeedback {
  ...
  return {
    ...
    summary: '当前展示的是本地诊断结果：它会根据原文关键词覆盖、长度完整度和表达差异给出基础反馈。连接 AI 后端后，可获得更细的句法、搭配和地道度分析。',
    strengths: [
    ...
    issues: [
      {
        title: '地道度诊断需要 AI 后端支持',
        severity: 'medium',
        suggestion: '部署 server/index.ts 中的分析代理，并配置 DEEPSEEK_API_KEY。',
        explanation: '前端不再保存 API Key，因此静态 GitHub Pages 环境会回退到本地诊断。',
      },
      {
        title: '重点复盘原文表达',
        severity: 'low',
